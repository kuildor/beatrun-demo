...
local TEX_SIZE = 512
local tex = GetRenderTarget( "PerlinCity", TEX_SIZE, TEX_SIZE )
local myMat = CreateMaterial( "PerlinCityMat", "UnlitGeneric", {
...
function perlin.randomize()
	...
	cam.Start2D()
		...
		b=0
		for i=0, 100*100 do
			p[i] = p[i] or perlin:noise(b*0.01,a*0.01)*255
			local noise = p[i]
			if i%100==0 then
				b=0 a=a+5
			end
			surface.SetDrawColor(noise,noise,noise)
			surface.DrawRect(0+b,0+a,5,5)
			b=b+5
		end
		-- render.BlurRenderTarget( tex, 0.5, 0.5, 10 )
	cam.End2D()
